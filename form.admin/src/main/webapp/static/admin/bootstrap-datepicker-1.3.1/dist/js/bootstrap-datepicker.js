...
var Datepicker = function(element, options){
	...
	if(this.o.calendarWeeks) this.picker.find('tfoot .today, tfoot .clear').attr('colspan', function(i, val){ return parseInt(val) + 1; });
...
Datepicker.prototype = {
	...
	_buildEvents: function(){
		...
		this._secondaryEvents = [
			...
			[$(document), {
				mousedown: $.proxy(function(e){ //Clicked outside the datepicker, hide it
					if(!(this.element.is(e.target) || this.element.find(e.target).length || this.picker.is(e.target) || this.picker.find(e.target).length)) {
						//npapadopoulos: Fire hide Event When Click Outside Too
						//$(this.picker).hide();
						this.hide();
					}
				}, this)
				...
				case 'th':
					switch (target[0].className){
						case 'datepicker-switch':
							//npapadopoulos: Disable Month/Year/Decade Switch Functionality 
							//this.showMode(1);
							break;
						case 'prev':
						case 'next':
	...
	moveMonth: function(date, dir){
		...
		if(mag === 1){
			test = dir === -1
				// If going back one month, make sure month is not current month (eg, Mar 31 -> Feb 31 == Feb 28, not Mar 02)
				? function(){
					return new_date.getUTCMonth() === month;
				}
				// If going forward one month, make sure month is as expected (eg, Jan 31 -> Feb 31 == Feb 28, not Mar 02)
				: function(){
		...
		// Common date-resetting loop: if date is beyond end of month, make it end of month
		while (test()){
			new_date.setUTCDate(--day);
			new_date.setUTCMonth(new_month);
		}
			...
			case 13: //Enter
				...
				if(this.picker.is(':visible')){
					...
					if(typeof e.stopPropagation === 'function') e.stopPropagation();
						else e.cancelBubble = true; // IE6,7,8 ignore "stopPropagation"
...
DateRangePicker.prototype = {
	...
	dateUpdated: function(e){
		//`this.updating` is a workaround for preventing infinite recursion between `changeDate` triggering and `setUTCDate` calling
...
var DPGlobal = {
	...
	parseFormat: function(format){
		// IE treats \0 as a string end in inputs (truncating the value), so it's a bad format delimiter, anyway
...
/* DATEPICKER DATA-API */
$(document).on(
	'focus.datepicker.data-api click.datepicker.data-api', '[data-provide="datepicker"]',
	function(e){
		var $this = $(this);
