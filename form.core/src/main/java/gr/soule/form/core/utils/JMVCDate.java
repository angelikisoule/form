...
public class JMVCDate {
    ...
    /**
     * Produces A Human Readable Time Span Description (i.e.: 1 year 2 months 1 week 2 days 2 hours 21 minutes)
     * @param startDate The Starting Date Of The Time Span
     * @param endDate The Ending Date Of The Time Span
     * @param lowestUnit The Lowest Time Unit We Need To Display. If for example You Choose DAY the Time span Will Not Display Hours Or Seconds
     * @return String With The Time Span In The Requested Format
     */
    public static String timespan(Date startDate, Date endDate, TIMESPAN lowestUnit) {
        ...
        lowestUnit = TIMESPAN.MIN;
        
        if(timestamp > months) {
        	lowestUnit = TIMESPAN.MONTH;
        }
        else if(timestamp > days) {
            lowestUnit = TIMESPAN.DAY;
        }
        else if (timestamp > hours) {
            lowestUnit = TIMESPAN.HOUR;
        }
        else if (timestamp < minutes) {
            lowestUnit = TIMESPAN.SEC;
        }
    ...
    public static Date stringToDate(String dateStr, String pattern) {
        ...
        catch(ParseException exception) { //Do Nothing
